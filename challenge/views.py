...
def submit_message(request):
    if request.method == "POST":
        ...
        async_task(
            send_email,
            {"subject": subject, "email": email, "name": name, "message": msg},
        )

        return HttpResponse(
            f"""<div class="success-message">✅ Thank you {escape(name)}! Your message has been sent.</div>"""
        )
    else:
        return HttpResponse("")
